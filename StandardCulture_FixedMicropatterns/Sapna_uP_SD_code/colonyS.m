    ...
    methods
        ...
        function object = calculateRadialProfile(object, meta, bins, tooHighIntensity, colonyMask, nuclearMask)
            ...
            if ~isempty(find(colonyMask, 1)) % for regular dishes some images are empty
                ...
                %%
                %                 intensityProfile = getColonyNuclearIntensityProfile(images, nuclearMask, meta); % colony nuclear intensity
                %                 nonNuclearChannel = find(ismember(upper(meta.channelLabel), 'NON_NUCLEAR'), 1); % colony non-nuclear intensity
                %                 if ~isempty(nonNuclearChannel)
                %                     nonNuclearIntensityProfile = getColonyNuclearIntensityProfile(images(:,:,nonNuclearChannel), nonNuclearMask, meta);
                %                     intensityProfile(:,:, nonNuclearChannel) = nonNuclearIntensityProfile;
                %                 end
                %                 spotsChannel = find(ismember(upper(meta.channelLabel), 'SPOTS'), 1);
                %                 intensityProfile(:,:,spotsChannel) = getColonyNuclearIntensityProfile(images, nuclearMask, meta);
                %                 intensityProfile_dapiNormalized = intensityProfile;
                %                 intensityProfile_dapiNormalized(:,:,nuclearChannels) = intensityProfile(:,:,nuclearChannels)./intensityProfile(:,:,dapiChannel);
                %                 intensityProfile_dapiNormalized(isnan(intensityProfile_dapiNormalized)) = 0;
